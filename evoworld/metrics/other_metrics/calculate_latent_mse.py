...
# Function to calculate latent MSE between image tensors
def calculate_latent_mse(image1_tensor, image2_tensor, model=None, transform=None):
    """Calculate the latent MSE between two image tensors."""
    ...
    # image1_tensor = preprocess_image(image1_tensor, transform)
    # image2_tensor = preprocess_image(image2_tensor, transform)
    image1_tensor = image1_tensor.reshape(-1, c, h, w)
    image2_tensor = image2_tensor.reshape(-1, c, h, w)
    image1_tensor = transform(image1_tensor)
    image2_tensor = transform(image2_tensor)

    latent_features1 = extract_features(model, image1_tensor)
    latent_features2 = extract_features(model, image2_tensor)

    # Reshape latent features back to original shape
    _, latent_c = latent_features1.shape
    latent_features1 = latent_features1.reshape(batch_size, n_frames, latent_c)
    latent_features2 = latent_features2.reshape(batch_size, n_frames, latent_c)

    # Calculate the mean MSE between latent features for each frame
    mse_result = np.mean((latent_features1 - latent_features2) ** 2, axis=(0, 2))
    # Calculate the standard deviation of MSE between latent features for each frame
    std_result = np.std((latent_features1 - latent_features2) ** 2, axis=(0, 2))
    ...
    result = {
        ...
        "value_std": std,
        "video_setting": image1_tensor.shape,
...
def calculate_average_latent_mse_across_all_images(folder_path, model, transform):
    ...
    for subfolder in tqdm(subfolders, desc="Processing images"):
        navigated_image_path = os.path.join(subfolder, "navigatied.png")
