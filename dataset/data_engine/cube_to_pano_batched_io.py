...
def load_cube_faces(cube_path):
    """Load cube faces for a single cube map."""
    faces = {}
...
def process_batch(cube_paths, output_path, width, height):
    """
    ...
    with Pool() as pool:
        cube_faces_list = pool.map(load_cube_faces, cube_paths)
    ...
    save_args = [
        (os.path.join(output_path, f'{os.path.basename(cube_paths[i])}.png'), panoramas[i])
        for i in range(len(cube_paths))
    ]
    num_processes=8
...
def main():
    ...
    # Collect all valid cube map directories
    cube_paths = [
        os.path.join(args.image_path, cube_id)
        for cube_id in os.listdir(args.image_path)
        if os.path.isdir(os.path.join(args.image_path, cube_id))
        and not cube_id.endswith('panorama')  # Exclude unwanted directories
    ]
    # Process in batches
    batch_size = args.batch_size
    for i in tqdm(range(0, len(cube_paths), batch_size)):
        batch = cube_paths[i:i + batch_size]
        process_batch(batch, args.output_path, args.width, args.height)
